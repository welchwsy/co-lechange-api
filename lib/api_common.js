...
class API {
  ...
  constructor(appid, appsecret, getToken, saveToken, tokenFromCustom) {
    ...
    this.prefix = 'https://openapi.lechange.cn/openapi/';
   ...
   * 设置urllib的hook
   */
  async request(url, params = {}, opts = {}, retry = 3) {
    if (typeof retry === 'undefined') {
      retry = 3;
    }
    const time = parseInt(new Date() / 1000);
    const nonce = crypto.createHash('md5').update(new Date() + '' + parseInt(Math.random() * 1000000)).digest('hex');
    var options = {
      system: {
        ver: '1.0',
        appId: this.appid,
        sign: crypto.createHash('md5').update(`time:${time},nonce:${nonce},appSecret:${this.appsecret}`).digest('hex'),
        time: time,
        nonce: nonce
      },
      id: crypto.createHash('md5').update(new Date() + '').digest('hex'),
      params: params
    };
    ...
    var res = await httpx.request(url, postJSON(options));
    if (res.statusCode < 200 || res.statusCode > 204) {
      var err = new Error(`url: ${url}, status code: ${res.statusCode}`);
      err.name = 'LeChangeAPIError';
      throw err;
    }
    ...
    if (contentType.includes('application/json') || contentType.includes('text/plain')) {
      ...
      if (data && data.result && data.result.code !== '0') {
        let err = new Error(data.result.msg);
        err.name = 'LeChangeAPIError';
        err.code = data.result.code;

        if ((err.code === 'TK1002' || err.code === 'TK1003') && retry > 0 && !this.tokenFromCustom) {
          // 销毁已过期的token
  ...
  async getAccessToken() {
    var url = this.prefix + 'accessToken';
    var data = await this.request(url);
    // 过期时间，因网络延迟等，将实际过期时间提前10秒，以防止临界点
    var expireTime = (new Date()).getTime() + ((data.expireTime * 1 - 10) * 1000);
    var token = new AccessToken(data.accessToken, expireTime);
